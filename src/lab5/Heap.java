...
public class Heap<T extends Comparable<T>> {
    ...
    private void heapifyUp() {
        // TODO
    	int thisIndex = container.size()-1;
    	int parentIndex = this.getParentIndex(thisIndex);
    	
    	
    	while (this.hasParent(thisIndex) &&  
    			container.get(thisIndex).compareTo(container.get(parentIndex)) < 0) {
    				T temp = container.get(thisIndex);
    				container.set(thisIndex, container.get(parentIndex));
    				container.set(parentIndex, temp);
    				thisIndex = parentIndex;
    				parentIndex = this.getParentIndex(thisIndex);
    			}
